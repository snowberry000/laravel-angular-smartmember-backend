    ...
    public function directory(){
        ...
        if(\Input::has('featured')){
            ...
            foreach ($sub_categories as $key => $value) {
                $results[$value] = Directory::whereNull('deleted_at')->whereNotNull('image')->where('is_visible' , true)->where('sub_category' , $value)->with(['site' , 'site.owner' , 'site.meta_data' => function($query) { $query->where('key', '=', 'logo_url');}])->orderBy('total_revenue','desc')->take(4)->get();
            }
