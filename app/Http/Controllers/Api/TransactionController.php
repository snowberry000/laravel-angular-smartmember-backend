...
class TransactionController extends SMController
{
    ...
    public function summary(){
        ...
        $summary["total_today_sales"] = $this->model->getOne("select SUM(price) as total_today_sales from transactions WHERE DATE(created_at) = CURDATE() and site_id = " . $this->site->id . " and type != 'rfnd'" , 'total_today_sales'); 
        $summary["total_yesterday_sales"] = $this->model->getOne("select SUM(price) as total_yesterday_sales from transactions WHERE DATE(created_at) = DATE(DATE_SUB(NOW(), INTERVAL 1 DAY)) and site_id = " . $this->site->id . " and type != 'rfnd'" , 'total_yesterday_sales'); 
        ...
        $summary["number_of_today_sales"] = $this->model->getOne("select count(*) as number_of_today_sales from transactions WHERE DATE(created_at) = CURDATE() and site_id = " . $this->site->id . " and type != 'rfnd'" , 'number_of_today_sales'); 
        $summary["number_of_yesterday_sales"] = $this->model->getOne("select count(*) as number_of_yesterday_sales from transactions WHERE DATE(created_at) = DATE(DATE_SUB(NOW(), INTERVAL 1 DAY)) and site_id = " . $this->site->id . " and type != 'rfnd'" , 'number_of_yesterday_sales'); 
