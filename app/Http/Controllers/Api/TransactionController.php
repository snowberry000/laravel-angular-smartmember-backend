...
class TransactionController extends SMController
{
    ...
    public function summary(){
        ...
        $summary["total_today_sales"] = $this->model->getOne("select SUM(price) as total_today_sales from transactions WHERE (created_at BETWEEN CONCAT(CURDATE(),' 03:01:00') AND CONCAT( DATE(DATE_ADD(NOW(), INTERVAL 1 DAY)) ,  ' 2:59:00' ) )  and site_id = " . $this->site->id . " and type != 'rfnd'" , 'total_today_sales');
        $summary["total_yesterday_sales"] = $this->model->getOne("select SUM(price) as total_yesterday_sales from transactions WHERE (created_at BETWEEN CONCAT( DATE(DATE_SUB(NOW(), INTERVAL 1 DAY)) ,  ' 03:01:00' )  AND CONCAT( CURDATE() ,  ' 2:59:00' ) ) and site_id = " . $this->site->id . " and type != 'rfnd'" , 'total_yesterday_sales');
        ...
        $summary["number_of_today_sales"] = $this->model->getOne("select count(*) as number_of_today_sales from transactions WHERE (created_at BETWEEN CONCAT(CURDATE(),' 03:01:00') AND CONCAT( DATE(DATE_ADD(NOW(), INTERVAL 1 DAY)) ,  ' 2:59:00' ) ) and site_id = " . $this->site->id . " and type != 'rfnd'" , 'number_of_today_sales');
        $summary["number_of_yesterday_sales"] = $this->model->getOne("select count(*) as number_of_yesterday_sales from transactions WHERE (created_at BETWEEN CONCAT( DATE(DATE_SUB(NOW(), INTERVAL 1 DAY)) ,  ' 03:01:00' )  AND CONCAT( CURDATE() ,  ' 2:59:00' ) ) and site_id = " . $this->site->id . " and type != 'rfnd'" , 'number_of_yesterday_sales');
