...
class EmailJobController extends SMController
{
    ...
    public function index()
    {
        ...
        $email_setting = EmailSetting::whereSiteId($this->site->id)->first();
		...
		if( count( $email_jobs ) > 0 )
		{
			foreach ($email_jobs as $email_job)
			{
				...
				$email_job->click_count = Click::leftJoin('links', 'clicks.link_id', '=' , 'links.id')->where('links.job_id','=',$email_job->id)->count();
				$email_job->unsubscriber_count = Unsubscriber::whereJobId($email_job->id)->count();
				$queue_items = EmailQueue::whereJobId($email_job->id)->whereNull('deleted_at')->count();
				if ($queue_items == 0)
				{
                ...
                if( !isset( $email_setting )&&empty( $email_setting )&&$email_job->status!="Sent Successfully" )
                {
                    $email_job->sendgrid_account_check = true;
                }
                elseif( ($email_setting->username == '' || $email_setting->password == '')&&$email_job->status!="Sent Successfully" )
                {
                    $email_job->sendgrid_account_check = true;
                }
                elseif( ($email_setting->full_name == '' || $email_setting->sending_address == '')&&$email_job->status!="Sent Successfully" )
                {
